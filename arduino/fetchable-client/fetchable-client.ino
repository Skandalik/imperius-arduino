...
void fetchSensorStatus() {
  ...
  do  {
    sensorStatus = dht.readHumidity();
  } while (isnan(sensorStatus));
  Serial.print("Humidity: ");
  Serial.print(sensorStatus);
  Serial.print(". String: ");
  statusString = sensorStatus;
...
void sendRegisterBroadcastMessage(PubSubClient client) {
  ...
  fetchSensorStatus();
    ...
    sensorInit["status"] = statusString.c_str();
...
void setupMqtt()
{
  ...
  checkStatusTopic = TOPIC_PREFIX + sensorIdToEeprom + "/status/check";
...
void callback(String topic, byte *message, unsigned int length)
{
  ...
  if (topic.equals(allCheckStatusTopic) || topic.equals(checkStatusTopic))
  {
    fetchSensorStatus();

    sensor["uuid"] = sensorId.c_str();
    sensor["status"] = statusString.c_str();
...
void reconnect()
{
  ...
  while (!client.connected())
  {
    ...
    if (client.connect(sensorIdToEeprom))
    {
      ...
      client.subscribe(checkStatusTopic.c_str());
